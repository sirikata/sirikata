...
class InputStream(object):
  ...
  def ReadLittleEndian32(self):
    ...
    """
    try:
      i = int(self._buffer[self._pos]);
      i += 256*int(self._buffer[self._pos+1]);
      i += 256*256*int(self._buffer[self._pos+2]);
      i += 256*256*256*int(self._buffer[self._pos+3]);
      self._pos += 4
      return i  # unpack() result is a 1-element tuple.
    except:
  ...
  def ReadLittleEndian64(self):
    ...
    try:
      ...
      i += 256*256*int(self._buffer[self._pos+2]);
      i += 256*256*256*int(self._buffer[self._pos+3]);
      i += 256*256*256*256*int(self._buffer[self._pos+4]);
      i += 256*256*256*256*256*int(self._buffer[self._pos+5]);
      i += 256*256*256*256*256*256*int(self._buffer[self._pos+6]);
      i += 256*256*256*256*256*256*256*int(self._buffer[self._pos+7]);
      self._pos += 8
      return i  # unpack() result is a 1-element tuple.
