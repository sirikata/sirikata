...
(function()
 {
    ...
    function tripletIsValueType (triplet)
    {
     ...
     function restoreFunction(keyName,unfixedFunc,ptrId,ptrsToFix,nameService, cb)
     {
         ...
         {
             system.print('ERROR: Error in restoreFunction.  Trying to restore a function with faulty syntax. Key: ' + keyName + '\n');
             returner = function(){};
             success = false;
         }
         nameService.insertObjectWithName(returner,ptrId);
     ...
     std.persist.restoreFromAndGetNamesAsync = function (keyName,id,cb)
     {
         ...
         // Setup a timeout in case we fail to get all presences reconnected
         var restoreCheckback = function() {
             cb(false);
         };
         var tout = system.timeout(60,restoreCheckback);

         fixReferences(
             keyName, id,ptrsToFix,nameService,
             std.core.bind(finishRestoreFromAndGetNamesAsync, undefined, keyName, id, cb, ptrsToFix, nameService, tout) // + success, obj
         );
     };
