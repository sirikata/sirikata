...
v8::Handle<v8::Value> JSUtilStruct::struct_createWhenWatchedList(v8::Handle<v8::Array>arrayOfItems)
{
    ...
    {
        create a new WhenWatchedItem and return it as an object;
        JSWhenWatchedItemStruct* wwis = new JSWhenWatchedItemStruct(associatedObjScr);
        return associatedObjScr->createWhenWatchedItem(wwis);
    }

    //decode first argument as watched list
    ...
    JSWhenWatchedItemStruct* toMergeWith = JSWhenWatchedItemStruct::decodeWhenWatchedItemStruct(arrayOfItems->Get(0), errorMessage);

    if (toMergeWith == NULL)
        return v8::ThrowException(v8::Exception::Error(v8::String::New(errorMessage.c_str(),errorMessage.length())));

    //merge all of the watched items with each other.
    for (int s=1; s < (int) arrayOfItems->Length(); ++s)
    {
        String errorMessage = "Error decoding item from array in struct_createWhenWatchedList.  Should be a whenWatchedItem, but is not.  ";
        ...
        singleItem->mergeItems(toMergeWith);
    }

    return arrayOfItems->Get(0);
}
