...
void JSWhenStruct::setPredTimer()
{
    ...
    if (whenPeriod !=  WHEN_PERIOD_NOT_SET)
    {
        if (whenPeriod < WHEN_MIN_PERIOD)
        {
            SILOG(error,js,"Error in setPredTimer of JSWhenStruct.cpp: trying to set when period below its min period.  Setting predicate checking period to the min checking period.");
            whenPeriod = WHEN_MIN_PERIOD;
        }

        mDeadlineTimer->expires_from_now(boost::posix_time::seconds(whenPeriod));
...
void JSWhenStruct::deadlineExpired()
{
    if (stateSuspended)
    {
        SILOG(error,js,"Error in deadlineExpired of JSWhenStruct.  Should have been in a suspended state, but received a callback from timer constructor of JSWhenStruct.cpp: trying to set when statement below its min period.  Setting predicate checking period to the min checking period.");
        return;
    }
...
bool JSWhenStruct::evalPred()
{
    ...
    if (! returnedBool)
    {
        SILOG(js,error,"Error in evalPred of JSWhenStruct.cpp.  Predicate did not return bool.  Suspending when statement");
        suspend();
        return false;
    }
...
v8::Handle<v8::Value>JSWhenStruct::struct_whenSetState(bool boolArg)
{
    stateTrue  = boolArg;
    return v8::Undefined();
}

v8::Handle<v8::Value>JSWhenStruct::struct_whenGetState()
{
    v8::HandleScope handle_scope;
    return v8::Value::Boolean::New(boolArg);
}

v8::Handle<v8::Value>JSWhenStruct::struct_setPeriod(double newPeriod)
{
    currentPeriod = newPeriod;
    cancel currentTimer;  Reset it;
    return v8::Undefined();
}
...
{
    v8::HandleScope handle_scope;
    return v8::Value::Number::New(currentPeriod);
}

v8::Handle<v8::Value>JSWhenStruct::struct_isSuspended()
{
    v8::HandleScope handle_scope;
    return v8::Value::Boolean::New(stateSuspended);
}

void JSWhenStruct::removeWatchablesFromScript()
{
    for (WatchableMapIter iter = mWatchables.begin(); iter != mWatchables.end(); ++iter)
...
v8::Handle<v8::Value>JSWhenStruct::struct_whenResume()
{
    ...
    mDeadlineTimer->async_wait(std::tr1::bind(&JSWhenStruct::evaluatePredicate,this));
