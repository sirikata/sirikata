...
public:
    ...
    NodeData merge(const NodeData& other) const {
        ...
        // Only non-empty if one of them wasn't empty
        result.mEmptyFilter = (result.mEmptyFilter && other.mEmptyFilter);
        return result;
    }
    ...
    void mergeIn(const NodeData& other, uint32 currentChildrenCount) {
        ...
        mEmptyFilter = (mEmptyFilter && other.mEmptyFilter);
    }
    ...
    bool satisfiesConstraints(const Vector3& qpos, const BoundingSphere& qregion, const float qmaxsize, const SolidAngle& qangle, const float qradius) const {
        ...
        assert(false && "satisfiesConstraints is not properly overridden for TermBloomFilterNodeData");
    ...
    void verifyChild(const NodeData& child) const {
        ...
        if (!child.mFilter.subsetOf(mFilter)) {
            printf("Found non-subset bloom filter in child\n");
        }
    }
    ...
    const Twitter::TermBloomFilter& getBloomFilter() const {
        return mFilter;
    }
    const bool getBloomFilterEmpty() const { return mEmptyFilter; }
private:
    float mMaxRadius;
    Twitter::TermBloomFilter mFilter;
    // An extra flag to indicate that we still have an empty bloom filter. This
    // is really only handy to hack around improperly built trees, especially
    // the bogus entries generated by local-only top-level pinto trees, where no
    // filter data will be provided. This allows us to treat those as
    // "always-satisfies" nodes so we actually get down to nodes with real data.
    bool mEmptyFilter;
};
