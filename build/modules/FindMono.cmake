...
IF(MONO_FOUND)
      ...
      STRING(REGEX REPLACE ".*([0-9]+)\\.([0-9]+)\\.([0-9]+)\\.([0-9]+).*" "\\1 \\2 \\3 \\4" MONO_VERSION ${MONO_STRING_VERSION})
      ...
      ELSE()
        STRING(REGEX REPLACE ".*([0-9]+)\\.([0-9]+)\\.([0-9]+).*" "\\1 \\2 \\3" MONO_VERSION ${MONO_STRING_VERSION})
        SEPARATE_ARGUMENTS(MONO_VERSION)
        LIST(LENGTH MONO_VERSION MONO_VERSION_LENGTH)
        IF(MONO_VERSION_LENGTH EQUAL 3)
          ...
          LIST(GET MONO_VERSION 2 MONO_SUBMINOR_VERSION)
        ELSE()
          STRING(REGEX REPLACE ".*([0-9]+)\\.([0-9]+).*" "\\1 \\2" MONO_VERSION ${MONO_STRING_VERSION})
          SEPARATE_ARGUMENTS(MONO_VERSION)
          LIST(LENGTH MONO_VERSION MONO_VERSION_LENGTH)
          IF(MONO_VERSION_LENGTH EQUAL 2)
            SET(MONO_MAJOR_VERSION)
            SET(MONO_MINOR_VERSION)
