...
./cbr --id=1 --layout="<$num,$num,1>" --duration=$duration --bandwidth=$bandwidth  --flatness=.001 --capexcessbandwidth=false --object.queue=fairfifo --serverips=serverip-$num-$num.txt --analysis.windowed-bandwidth=packet 
./cbr --id=1 --layout="<$num,$num,1>" --duration=$duration --bandwidth=$bandwidth  --flatness=.001 --capexcessbandwidth=false --object.queue=fairfifo --serverips=serverip-$num-$num.txt --analysis.windowed-bandwidth=datagram
...
wait
export datadir="d-$numnum-$bandwidth-$flatness-"`date +%H-%M-%S-%Y%m%d`
mkdir $datadir
mv trace-* $datadir
mv sync-* $datadir
